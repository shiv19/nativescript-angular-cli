...
import { BlueprintManagerJS, TemplateMetadata } from "./blueprint-manager-js";
...
export class GeneratorServiceJS {
    constructor(
        private $fs: IFileSystem,
        private $blueprintManagerJS: BlueprintManagerJS
    ) {}
    ...
    public generate(name: string, blueprintType: string, relativePath: string) {
        ...
        console.log(
            `Prepared classifiedName: ${classifiedName} dasherizedName: ${
                dasherizedName
            }`
        );

        const items = this.$blueprintManagerJS.listTemplates(blueprintType);
    ...
    private generateFiles(
        ...
        templates.forEach(template => {
            const fileUrl = this.parseFileUrl(
                relativePath,
    ...
    private parseData(
        ...
        return data
            .replace(classifiedRegExp, classifiedName)
            .replace(dasherizedRegExp, dasherizedName);
    }
...
$injector.register("generatorServiceJS", GeneratorServiceJS);
