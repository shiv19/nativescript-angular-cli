...
class GenerateComponentCommand {
	...
	canExecute(args: string[]) {
		return new Promise((resolve, reject) => {
			...
			this.$errors.failWithoutHelp(`You should pass at least one argument to 'generate component' command.`);
		});
	}
	...
	execute(args: string[]) {
		return new Promise((resolve, reject) => {
			...
			const message = this.$generatorService.generateComponent(args[0]);
